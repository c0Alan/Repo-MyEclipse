...
// @author Artery
// @date 2011-10-19
// @id 49a7f555aed2bd77bbc2befefb6cbeed
// **********************************************//
...
function button3dddd_onClickClient(rc) {
	alert(Artery.get("faOrgan79d00").getValue());
}
...
function buttond954e_onClickClient(rc) {
	alert(Artery.get("faOrgan79d00").getValueText());
}
...
function buttonda1c1_onClickClient(rc) {
	Artery.get("faOrgan79d00").setValue("-2");
}
...
function button5f155_onClickClient(rc) {
	Artery.get("faOrgan79d00").setOriginalValue("-2");
}
...
function buttoncd37b_onClickClient(rc) {
	Artery.get("faOrgan79d00").setLabel("设置的标签");
}
...
function buttonb43cf_onClickClient(rc) {
	Artery.get("faOrgan79d00").setLabelColor("red");
}
...
function button3cbf8_onClickClient(rc) {
	Artery.get("faOrgan79d00").setLabelStyle("background-color:blue;");
}
...
function buttonc6030_onClickClient(rc) {
	Artery.get("faOrgan79d00").setLabelCntStyle("background-color:green;");
}
...
function button5e466_onClickClient(rc) {
	Artery.get("faOrgan79d00").setValueStyle("color:blue;");
}
...
function button55e17_onClickClient(rc) {
	if (Artery.get("faOrgan79d00").allowBlank) {
		Artery.get("faOrgan79d00").setRequired(true);
	} else {
		Artery.get("faOrgan79d00").setRequired(false);
	}
}
...
function buttonb633c_onClickClient(rc) {
	Artery.get("faOrgan79d00").disable();
}
...
function button34f1a_onClickClient(rc) {
	Artery.get("faOrgan79d00").enable();
}
 ...
 *            控件改变后的新值
 */
function faOrgan71526_onChangeClient(rc, oldValue, newValue) {
 ...
 *            控件的值
 */
function faOrgan881d9_onValidClient(rc, value) {
	rc.put('value', value);
	rc.asyn = false;
	rc.send();
 ...
 */
function faOrgana4ef5_onKeyupClient(rc, value, eventObject) {
...
/**
 * onClickClient(button8735f)
 * 
 * @param  rc 系统提供的AJAX调用对象
 */
function button8735f_onClickClient (rc){
	alert(Artery.get("faOrgan0cb5f").getValue());
}
/**
 * onClickClient(buttoncf2d4)
 * 
 * @param  rc 系统提供的AJAX调用对象
 */
function buttoncf2d4_onClickClient (rc){
	alert(Artery.get("faOrgan2a4f2").getValue());
}
/**
 * onClickClient(button5f502)
 * 
 * @param  rc 系统提供的AJAX调用对象
 */
function button5f502_onClickClient (rc){
	Artery.get("faOrgan0cb5f").setValue("user_-2");
}
/**
 * onClickClient(button7ec71)
 * 
 * @param  rc 系统提供的AJAX调用对象
 */
function button7ec71_onClickClient (rc){
	Artery.get("faOrgan2a4f2").setValue("user_-2");
}